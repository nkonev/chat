...
func (ch *AuthorizationService) CheckAccess(ctx context.Context, params map[string]string) int {
	...
	if err != nil {
		ch.lgr.ErrorContext(ctx, "Error checking access", "err", err)
	...
	if err != nil {
		ch.lgr.InfoContext(ctx, "Unable to get userId", "err", err) // it can be error when overrideChatId and overrideMessageId are missed
		return http.StatusUnauthorized
	}
